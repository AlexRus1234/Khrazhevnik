...
> Архив разработки до 1.0.0 включительно (старый формат «волны»).
> Актуальные записи — [CHANGELOG.md](CHANGELOG.md), перевод —
...
## Дашборд-v2 (unreleased)

Волна по предложениям владельца 2026-09-07: наблюдаемость дашборда —
обновление статистики без перезагрузки, per-eco разрез «пакетный
менеджер → статистика», персистентность счётчиков и сброс, история
клиентских транзакций (сессии 91–102).

- **Web (сессия 91):** кнопка «Обновить» в панели кеша дашборда —
  перечитывание статистики и задач без перезагрузки страницы.
...
- **БД (сессия 95):** миграция 0007 `cache_stats` на трёх драйверах —
  персистентный снапшот per-eco счётчиков статистики (переживает
  рестарт); порт `StatsStore` (сохранение снапшота одной транзакцией /
  чтение / сброс) — потребители в сессии 96.
- **Движок (сессия 96):** счётчики статистики переживают рестарт —
  statskeeper сеет снапшот `cache_stats` в per-eco атомики при старте
...
- **Web (сессия 98):** кнопка «Сбросить статистику» в панели кеша
  дашборда — confirm, затем POST reset и перечитывание статистики
  без перезагрузки страницы.
- **Движок (сессия 99):** кольцевой буфер последних 50 клиентских
  транзакций кеша (время, экосистема, путь, исход HIT/MISS/STALE/
  error, размер, текст ошибки) — bounded-память, O(1) запись,
...
- **Web (сессия 101):** панель «Последние транзакции» на дашборде —
  таблица последних клиентских запросов к кешу (время, экосистема,
  путь, статус-бейдж, размер, ошибка); живая лента в существующем
  2-секундном поллинге задач, кнопка «Обновить» перечитывает немедленно.
- **Доки (сессия 102):** ROADMAP, ui.md ru/en, CHANGELOG и карта
  логов синхронизированы с волной — закрытие Дашборд-v2.

## Предрелизная полировка v0.9.3 (unreleased)

Микродополнения перед v0.9.3; далее — приёмка v1.0.0 по
[RELEASE.md](docs/RELEASE.md).

- **Web (сессия 103):** версия сборки в углу интерфейса — topbar рядом
  с названием и нижний угол экрана входа; источник — публичный
  GET /api/v1/ (поле `version` существовало, правка фронтенд-only).
- **Доки (сессия 103):** запуск через docker — `deploy/docker-compose.yml`
  + секции в [func/ru/deploy.md](docs/func/ru/deploy.md),
  [func/EN/deploy.md](docs/func/EN/deploy.md); страница реверс-прокси
  Caddy/Traefik/nginx (TLS, `trusted_proxies`, стриминг, лимиты
  upload) — func/{ru,EN}/reverse-proxy.md.
- **CI (сессия 103):** push OCI-образа в registry — только с релизных
  `v*`-тегов (сборка остаётся безусловной, `sha-*`-версии не
  пушатся): прежде каждый прогон оставлял новую версию в registry.

## Зеркало-форматы (unreleased)

Оживление зеркалирования по фактам владельца 2026-09-09 (сессии
103–107): Arch отдаёт sync-БД tar.gz, Fedora 41+/Leap 16.0 —
primary.xml.zst; прежде обе экосистемы зеркалироваться не могли.

- **Парсер pacman (сессия 103):** sync-БД `{repo}.db` читается и как
  tar.gz, и как tar.zst — декомпрессор выбирается по magic-байтам
  потока (имя файла компрессию не несёт); декомпресс-лимит 1 GiB
  действует на обе ветки.
- **pacman fallback (сессия 104):** legacy `.db.tar.gz` парсится при
  404 короткого `{repo}.db` (gzip-эпоха Arch и legacy-зеркала
  публикуют только его); отсутствие обоих — исходный NotFound,
  UnsupportedError из пути зеркалирования убран.
- **rpm-md zst (сессия 105):** Enumerate читает primary как
  несжатый, .gz и .zst (Fedora 41+/Leap 16.0 отдают
  `<sha>-primary.xml.zst`, .gz-варианта в repomd нет); zstd-ветка
  капнута 1 GiB (твин gzip-капа сессии 54), горутины декодера
  гасятся; .zst убран из unsupported (.zck/.xz/.bz2 — честная
  UnsupportedError).
- **Integration (сессия 106):** полный mirror sync против
  httptest-фикстур реальных форматов (pacman gzip-БД, rpm-md
  zst-primary) — от создания remote до HIT из кеша на публичном
  порту; регресс «формат upstream ≠ формат парсера» ловится
  сквозным тестом.
- **Доки (сессия 107):** SPECIFICATION/ROADMAP/TESTING
  синхронизированы с кодом — закрытие волны.

...
func TestLookupRemoteReloadBoundedByTimeout(t *testing.T) {
	...
	go func() {
		_, ok := a.Resolve("/apt/debian/Packages")
		done <- ok
	}()
	select {
	case ok := <-done:
...
func TestLookupRemoteReloadCounting(t *testing.T) {
	...
	store.rs = append(store.rs, domain.Remote{
		ID: 6, Name: "ubuntu", Ecosystem: "apt", BaseURL: "https://archive.ubuntu.com/ubuntu", Enabled: true,
	})
	clock.Advance(remoteCacheTTL)
	if _, ok := a.Resolve("/apt/ubuntu/Packages"); !ok {

...
func TestLookupRemoteReloadBoundedByTimeout(t *testing.T) {
	...
	go func() {
		_, ok := a.Resolve("/nix/cache/" + hash32 + ".narinfo")
		done <- ok
	}()
	select {
	case ok := <-done:
...
func TestLookupRemoteReloadCounting(t *testing.T) {
	...
	store.rs = append(store.rs, domain.Remote{
		ID: 6, Name: "mirror", Ecosystem: "nix", BaseURL: "https://mirror.example.com/nix", Enabled: true,
	})
	clock.Advance(remoteCacheTTL)
	if _, ok := a.Resolve("/nix/mirror/" + hash32 + ".narinfo"); !ok {

...
// Package xbps — адаптер экосистемы Void Linux (XBPS): маппинг путей
// публичного порта /xbps/<remote>/<остальной-путь> на upstream и
// классификация объектов по изменчивости. Кеш RemoteStore с
// инвалидацией по TTL 30с (KISS): новые remotes подхватываются без
// рестарта.
//
// Лэйаут Void плоский (не current/<arch>/, как ошибочно считалось до
// сессии 128): в корне репозитория лежат индекс <arch>-repodata
// (zstd ≤ level 9 → pax-tar из index.plist, index-meta.plist и
// stage.plist) и пакеты <pkgver>.<arch>.xbps с RSA-подписью
// <pkgver>.<arch>.xbps.sig2; публичный ключ встроен в index-meta.plist
// (TOFU-импорт клиентом) — см. docs/ROADMAP.md, «Void Linux (XBPS)».
// Имя файла пакета клиент строит сам из полей индекса, поэтому
// разбор repodata и Enumerate зеркала появляются отдельными сессиями
// волны (131–135), а не здесь.
//
// Имя экосистемы и URL-префикс совпадают: «xbps».
package xbps
...
// Периоды кеширования/ревалидации. remoteCacheTTL — TTL кеша
// RemoteStore (новые/изменённые remotes подхватываются без рестарта);
// TTL индексов mutable — инвариант экосистемы: <arch>-repodata
// меняется при каждом xbps-rindex репозитория, потому короткий TTL 5m.
const (
...
// Adapter реализует port.Ecosystem для xbps.
type Adapter struct {
...
func New(remotes port.RemoteStore, clock port.Clock) (*Adapter, error) {
	if remotes == nil {
		return nil, fmt.Errorf("xbps: RemoteStore обязателен")
	}
	if clock == nil {
		return nil, fmt.Errorf("xbps: Clock обязателен")
	}
	return &Adapter{
		remotes:       remotes,
		clock:         clock,
...
// URLPrefix возвращает префикс путей публичного порта. У xbps префикс
// совпадает с именем.
func (a *Adapter) URLPrefix() string { return Name }

// Resolve переводит /xbps/<remote-name>/<остальной-путь> в Target.
// false — путь не принадлежит xbps или remote неизвестен/выключен.
...
// classifyRules — таблица классификации xbps как данные. Порядок —
...
func classifyRules() []rawRule {
	...
	return []rawRule{
		// Immutable: подписанные .xbps пакеты и их подписи. Первыми —
		...
		// иначе **/*-repodata заматчил бы и вложенные пути.
		{"", `^[^/]+-repodata$`, indexMutable},
...
// Дубликат из apt/rpmmmd/pacman/apk: mod→mod запрещён, адаптеры
// самодостаточны.

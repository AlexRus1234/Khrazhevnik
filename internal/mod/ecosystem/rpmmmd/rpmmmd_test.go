...
func TestLookupRemoteReloadBoundedByTimeout(t *testing.T) {
	...
	go func() {
		_, ok := a.Resolve("/rpm/fedora/repodata/repomd.xml")
		done <- ok
	}()
	select {
	case ok := <-done:
...
func TestLookupRemoteReloadCounting(t *testing.T) {
	...
	store.rs = append(store.rs, domain.Remote{
		ID: 6, Name: "centos", Ecosystem: "rpm-md", BaseURL: "https://mirror.example.com/centos", Enabled: true,
	})
	clock.Advance(remoteCacheTTL)
	if _, ok := a.Resolve("/rpm/centos/repodata/repomd.xml"); !ok {

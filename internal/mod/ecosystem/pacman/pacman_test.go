...
func TestLookupRemoteReloadBoundedByTimeout(t *testing.T) {
	...
	go func() {
		_, ok := a.Resolve("/pacman/arch/core.db")
		done <- ok
	}()
	select {
	case ok := <-done:
...
func TestLookupRemoteReloadCounting(t *testing.T) {
	...
	store.rs = append(store.rs, domain.Remote{
		ID: 6, Name: "extra", Ecosystem: "pacman", BaseURL: "https://mirror.example.com/archlinux-extra", Enabled: true,
	})
	clock.Advance(remoteCacheTTL)
	if _, ok := a.Resolve("/pacman/extra/core.db"); !ok {

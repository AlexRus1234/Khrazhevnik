...
// адаптера. Значение никто не читает до сессии 156 (включение в
// doer-фабрику).
...
const sqlSettingsProxyGet = `SELECT value FROM settings WHERE key = $1`
...
func settingsUpsertSQL() string {
	return dbtalk.Upsert(dbtalk.Postgres{}, "settings", "key",

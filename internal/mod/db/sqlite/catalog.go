...
// запросом). Cap в Go, а не MIN(limit, 1000) в SQL, потому что
// MariaDB допускает в LIMIT только литералы и плейсхолдеры —
// выражений нет; капим одинаково во всех драйверах.
...
const (
	...
	sqlRemoteUpdate = `UPDATE remotes SET name = ?, ecosystem = ?, upstream_url = ?, proxy_url = ?, mode = ?, enabled = ?, sync_interval_sec = ?, include = ? WHERE id = ?`
	...
	sqlJobByRemote = sqlJobSelect + ` WHERE remote_id = ?`
...
func revocationUpsertSQL() string {
	return dbtalk.Upsert(dbtalk.SQLite{}, "revoked_sessions", "jti",

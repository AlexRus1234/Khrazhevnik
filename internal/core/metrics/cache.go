...
type Cache struct {
	...
	// Ranges — 206-ответы Range-раздачи per-eco (сессия 111). НЕ
	// персистится в cache_stats/БД: разовая сессия-факт, а ring-история
	// txn и так показывает 206-транзакции. Пишется через
	// ObserveRangeResponse; глобальной серии нет — только per-eco.
	Ranges atomic.Int64

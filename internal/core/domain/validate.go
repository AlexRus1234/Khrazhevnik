...
// [a-zA-Z0-9/._+-] и «~», «^», «:» (см. allowedKeyByte — реальные
// синтаксисы версий пакетов), без «..», без ведущего/хвостового «/»,
// без пустых сегментов, длиной до maxKeyLen. Верхний регистр допущен
// осознанно (сессия 19): пути реальных пакетов бывают case-чувствительными
...
func allowedKeyByte(b byte) bool {
	...
	case b == '+' || b == '~' || b == '^' || b == ':':
		...
		// №6), epoch-версии Arch (nftables-1:1.1.7-3…; инцидент
		// 2026-09-12: 400 на единственном пакете с epoch из 46,
		// транзакция pacman падала целиком). Инертны для traversal —
		// не кодируются, не образуют сегментов пути, легальны в
		// fs-именах unix и S3-ключах. «:» нелегален в именах NTFS
		// (ADS-разделитель) — fs-хранилище posix-only по дизайну.
		return true
	}
	return false
}

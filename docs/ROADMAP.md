...
с обеими волнами (64). Nix-smoke отложен —
...
будущего eviction — в разделе «Пост-v1 / далёкое будущее» ниже.
...
**Статус: запланирован (2026-09-07).** Волна по предложениям
...
не удаляются до eviction (пост-v1, см. раздел ниже).
...
## Пост-v1 / далёкое будущее

**Статус: запланировано.** Текущее
поведение не меняется — immutable-пакеты хранятся навсегда. Порядок
пост-v1:

- (а) расширение экосистем — void/xbps, pkg, Flatpak, Guix
  (binary cache протокола nix — narinfo/nar, свой base32 и ключи
  подписи; переиспользует наработки nix-адаптера);
- (б) только затем — настройка eviction старых версий: при
  кешировании новой версии пакета старая удаляется.

Границы (б) — чтобы будущая сессия не начинала ре-дизайн:

...
Нормативные правила (канон здесь; `AGENTS.md` — лишь on-disk шпаргалка
агентов и в git не входит): время — только через `port.Clock`;
...
(двойная регистрация — ошибка программиста); package-level `var`
запрещён как носитель **мутабельного** состояния — легальны только
иммутабельные значения: `cmd/khrazhevnik.Version`, закрытое состояние
`internal/core/registry` (compile-time реестр без него не собрать),
compile-time `embed.FS` (миграции) и sentinel-ошибки/таблицы,
инициализируемые один раз и не переприсваиваемые (уточнение ревью
2026-09-03: ~15 sentinel-`var` и таблицы taskregistry/apt/rpmmmd —
не нарушение); комментарии «почему», не «что».
...
- квоты — по сумме `Storage.List("repo/<id>/")` на каждый upload
  ...
  для чек-сумм — в сессии 17, когда s3 без List-обхода). При
  force-перезаписи старый размер вычитается из used (аудит 24:
  двойной счёт давал ложный отказ); повторная проверка — по
  фактическому размеру тела до Commit. Ошибка листинга — fail-closed
  (upload отклоняется): молчаливый «пустой» обход занизил бы used и
  пропустил бы перелимит;
- upload ключа сериализуется per-key in-flight-картой движка
  (аудит 24: TOCTOU Stat→Put — два параллельных upload тихо
  перезаписывали друг друга); второй параллельный — 409 `conflict`;
...
- Роль и `token_version` сверяются с БД **на каждом запросе** — роль из
  claim не доверяется.
- bcrypt: стоимость настраивается (`auth.bcrypt_cost`, 4–15, дефолт 12);
  guard на 72 байта при создании; на логине сравнение идёт ДО проверки
  длины (bcrypt обрезает до 72 байт — тайминг выравнивается), длинный
  пароль, прошедший сравнение, отклоняется явно. Несуществующий
  пользователь проверяет валидный dummy-хэш той же стоимости
  (тайминг-паритет, перечисление пользователей по таймингу не работает).
- Отзыв JWT-сессий персистентен (`revoked_sessions`): logout переживает
  рестарт процесса; in-memory карта — ограниченный fast-path.

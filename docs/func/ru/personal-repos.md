...
Путь приходит от клиента percent-экранированным (apt шлёт «+» как
`%2b`) — декод делается до проверки ключа, так что `%2b` и сырое `+`
дают один объект; двойное кодирование (`%252b`) — 400.

...
zchunk-метаданные (`.zck`) через прокси работают: если repomd
предлагает `.zck`-вариант, dnf5/librepo (дефолт) качает чанки
Range-запросами — lead одиночным диапазоном, тело —
`multipart/byteranges` до 256 частей. Хражевник отдаёт их `206`
byte-exact (волна «Range-206»); отдельный `.gz`-вариант для прокси не
нужен. Ограничение осталось только у **зеркала** (sync): `Enumerate`
`.zck` не разбирает — честная `UnsupportedError` (развилка 103–107).

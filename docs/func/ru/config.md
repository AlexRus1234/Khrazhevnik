...
один исходящий HTTP-клиент (`outboundHTTPClient`,
`cmd/khrazhevnik/wire.go`) со стандартным `http.ProxyFromEnvironment`.
Прокси задаётся обычными env-переменными Go — **без** префикса `KHRZ_`
и вне TOML:
...
на SOCKS-порт — это не сработает.

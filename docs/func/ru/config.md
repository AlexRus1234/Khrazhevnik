...
## Прокси исходящих запросов (upstream)

Все запросы к upstream — и кеш-прокси, и зеркальный sync — идут через
исходящий HTTP-клиент, выбираемый по прокси-строке источника
(`port.DoerFactory`, фабрика транспортов в `cmd/khrazhevnik/transport.go`).
Действует приоритет:

1. **Глобальная настройка** в БД (таблица `settings`, ключ
   `upstream.proxy`) — задаётся из веб-админки или `PUT
   /api/v1/settings/upstream-proxy`; значение `""` — настройка не задана,
   `direct` — без прокси, иначе URL прокси.
2. Если глобальная настройка пуста — **env-переменные** Go
   `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY` (стандартный
   `http.ProxyFromEnvironment` дефолтного клиента).
3. **Per-remote override** — поле `proxy_url` источника: пусто —
   наследовать глобальную настройку, `direct` — ходить напрямую в обход
   глобального прокси, иначе URL прокси только для этого источника.

Глобальная настройка применяется **без рестарта**: фабрика читает её
ленивым TTL-кешем (30с), поэтому смена из GUI вступает в силу не
позднее чем через 30с. Пароль прокси в аудит-лог не попадает — в detail
пишется замаскированный URL (`socks5://***@host:1080`). Настройка и
override видны на странице «Источники» (см. [ui.md](ui.md)).

### Переменные окружения (фолбэк)

Env-переменные используются, только когда глобальная настройка пуста.
Имена — без префикса `KHRZ_` и вне TOML:

| Переменная   | Назначение                                                        |
|--------------|-------------------------------------------------------------------|
| `HTTP_PROXY` | прокси для `http://`-upstream'ов                                  |
| `HTTPS_PROXY`| прокси для `https://`-upstream'ов                                 |
| `NO_PROXY`   | исключения (CSV: хосты, доменные суффиксы, CIDR) — ходят напрямую |

Нижний регистр имён (`https_proxy` и т.п.) тоже читается. Переменной
`ALL_PROXY` Go не читает — для «всего через один прокси» задавайте
`HTTP_PROXY` + `HTTPS_PROXY`.

### Формат URL прокси

Схемы URL: `http://`, `https://`, `socks5://`, `socks5h://` (для SOCKS5
эквивалентны — имя хоста резолвит прокси-сервер). Схема обязательна:
значение без схемы Go трактует как HTTP-прокси и пошлёт HTTP CONNECT
на SOCKS-порт — это не сработает. Тот же формат у глобальной настройки
и у поля `proxy_url` источника (валидация — `domain.ValidateProxyURL`,
потолок 2048 байт; `direct` — только для per-remote override).

Авторизация — userinfo в URL (`socks5://user:pass@host:port`):
SOCKS5 — логин/пароль RFC 1929, HTTP(S) — Basic. Спецсимволы
логина/пароля percent-encode'ьте, иначе парсер URL молча обрежет
значение.

Контейнер: всё исходящее через SOCKS5, loopback — напрямую:
...
`public_listen`/`admin_listen` прокси не касается. Исключение loopback
в `NO_PROXY` нужно утилитам рядом с приложением (curl self-bootstrap и
т.п.), которые читают те же переменные. Покрытие в CI — distro-test
с входом `use_socks_proxy`.

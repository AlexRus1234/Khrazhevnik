...
операций удобнее делать из [веб-админки](ui.md) — она использует этот
же API.
...
Сбой нашего стека (недоступны Storage или каталог БД при lookup репо)
— 503 `storage unavailable`; 502 — только за сбоем upstream: мониторинг
различает «сломан upstream» и «сломан инстанс» (сессия 50).

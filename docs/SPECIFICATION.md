...
1. **Кеш-прокси** — прозрачный pull-through для поддерживаемых пакетных
   ...
   побайтово, без переписывания). Запросы upstream идут с явным
   `Accept-Encoding: identity` (сессия 69, история L9); некомплаентный
   upstream, жмущий вопреки, — сохраняется и отдаётся byte-exact как
   есть (без перекодирования) — известное v1-ограничение; полное
   проксирование Content-Encoding upstream'а — пост-v1 (расширение
   ObjectMeta и схемы).
...
Прокси и публичный порт личных репо (:29202) понимают HTTP Range
(RFC 9110, волна «Range-206»); единая точка семантики —
`internal/core/web/range.go:serveRanged`. Ключ хранилища для срезов —
`port.Storage.GetRange(ctx, key, start, length)`: тело — независимый
byte-exact ридер подстроки `[start, start+length)`; `NotFoundError` —
нет объекта, `InvalidRangeError` — `start<0`, `length<=0` или выход за
размер. Инвариант побайтовой раздачи расширен: **срез byte-exact
объекта — byte-exact** (сегменты `.zck` librepo склеиваются только из
совпадающих байтов).

- **206 single** — ровно один диапазон (`bytes=10-19`, open-ended
  `bytes=100-`, суффикс `bytes=-64`): точные `Content-Range` и
  `Content-Length`, тело через stall-writer.
- **206 multipart/byteranges** — от 2 до `maxMultipartRanges` (256)
  диапазонов: часть = `Content-Range` + исходный Content-Type + байты,
  `Content-Length` просчитан до `WriteHeader`; части строго
  последовательны (в моменте один открытый ридер). Свыше 256 —
  200-полный (librepo режет `max_ranges` вдвое и сходится без расхода
  тела).
- **416** — ни один диапазон не пересекается с размером:
  `Content-Range: bytes */N`, тело не открывается.
- **200-полный** — синтаксический мусор (`bytes=abc`, `bytes=5-2`) и
  пустой Range: сервер MAY игнорировать Range (RFC 9110); а также
  несовпавший `If-Range`. `Accept-Ranges: bytes` выставляется на 200- и
  206-ответах Range-пути.
- **If-Range** (RFC 9110 §13.1.5, учитывается только при непустом
  Range): сильный ETag (точное сравнение строк; клиентский `W/"…"` с
  нашим сильным тегом не совпадает) ИЛИ HTTP-дата, равная
  `Last-Modified` (секунда UTC) → диапазон применяется; иначе —
  200-полный. Срез от другой версии байт опаснее полного тела: клиент
  склеит мусор и обвинит чексумму upstream.
- **STALE** — диапазон отдаётся из протухшей версии с `X-Cache: STALE`
  и `Warning` (как обычная stale-отдача); `X-Cache` присутствует и на
  206. Заголовки Range-пути (ETag/Content-Type/Last-Modified/
  Content-Length) совпадают с полной выдачей — контракт MISS↔HIT
  сессии 69 распространён на 206.
- **HEAD** с Range: заголовки 206/416 без открытия тела (0 байт в
  счётчиках).
- **Метрика** — `khrazhevnik_cache_range_responses_total{ecosystem}`:
  один инкремент на 206-ответ (multipart — один, не по частям); в
  `cache_stats`/БД не персистится (206-транзакции видны в ring-истории).
- **:29202** — то же поведение через `serveRanged`; мета берётся
  `Storage.Stat` только при наличии Range. У fs-объектов ETag нет —
  `If-Range` по тегу не срабатывает (деградация к 200-полному), по
  `Last-Modified` (mtime) работает; на s3 ETag есть и работает
  полностью. Per-eco счётчики байт/206 у :29202 не ведутся (публичный
  порт идёт мимо кеш-движка).

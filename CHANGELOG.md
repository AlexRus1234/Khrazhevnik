...
- Веб-UI: управление прокси источников (tri-state: наследовать
  глобальный / напрямую / свой URL) и глобальным прокси upstream на
  странице «Источники»; таблица показывает результирующий режим.
- Админ-API: табличный экспорт/импорт источников — GET
  `/api/v1/remotes/export` (текстовый файл `text/plain` с
  Content-Disposition) и POST `/api/v1/remotes/import` (построчный
  разбор, тело ≤256 KiB и ≤1000 строк). Импорт отдаёт 200 с отчётом
  `{created, skipped, errors}`: дубли по имени (в БД или внутри файла)
  и битые строки пропускаются с отчётом, валидные создаются; аудит
  `remote.import` со счётчиками.
- Web UI: кнопка «Экспорт» на странице «Источники» — скачивает
  актуальный список источников файлом `khrazhevnik-remotes.txt`
  (blob + `<a download>`).
- Веб-UI: панель импорта источников на странице «Источники» — вставка
  построчного текста или выбор `.txt`-файла; отчёт
  создано/пропущено/ошибки по строкам и обновление таблицы.

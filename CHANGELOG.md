...
- **Engine-аудит (сессия 63):** движковые записи аудита переживают
  ...
  и middleware.
- **Доки (сессия 70):** порядок пост-v1 в ROADMAP — новые
  экосистемы (void/xbps, pkg, Flatpak), затем eviction старых
  версий (proxy-режим, keep N≥2).
- **Прокси (сессия 82):** percent-escape в chi-wildcard — apt шлёт
  «+» в пути как %2b; экранированное и сырое написание дают один
  объект кеша, двойное кодирование и битые escape — 400 fail-closed.
- **Статистика кеша (сессия 83):** источник счётчиков — per-eco
  разрезы, глобальные значения stats/пром-серий — сумма per-eco на
  чтении; hits/bytes были всегда 0 (CI-факт №5 distro-test).
- **CI (сессии 72–73):** distro-test — opt-in job с реальными
  пакетными менеджерами 5 дистрибутивов против контейнера
  (DooD-сокет раннера); тест-образ Containerfile.test; критерий —
  HIT + cache-stats.
- **Доки (сессия 74):** TESTING/RELEASE/AGENTS синхронизированы
  с distro-test.

## Внешнее ревью 6 (unreleased)

Волна M4-Р8 — agentic-ревью opencode (2026-09-06, 3 параллельных
ревьюера; сессии 84–90).

- **Кеш (сессия 84):** неудачная запись меты mutable-замены после
  Commit больше не осирочивает блоб — writeKey уходит в фоновое
  удаление тем же механизмом, что и сменённые версии; индекс и
  старая версия не тронуты.
- **Кеш (сессия 85):** классификация 502/503 на write-пути —
  свойство движка (unavailableWriter в границе copyBody), а не
  дисциплина адаптера стораджа: сырая ошибка записи → 503.
- **Web (сессия 86):** обрыв клиента в прокси — Debug-лог (штатное
  событие write-deadline); ключ репо-объекта валидируется в
  web-слое до Storage.Get — единая точка против path-traversal.
- **Web (сессия 87):** имя действия аудита ставится до мутации —
  неудачная мутация пишется в аудит под настоящим именем действия.
- **Auth (сессия 88):** строгий ParseInt для `sub` JWT (префиксный
  Sscan глотал мусорный хвост); мёртвый ConstantTimeCompare убран.
- **Mod (сессия 89):** clamp MinInt64 в cryptoRand.Int64 s3 (twin
  fs); 32-бит гарды rpmheader в uint64-терминах.
- **Доки (сессия 90):** синхронизация ROADMAP/ARCHITECTURE/
  SPECIFICATION/TESTING с волнами Р5–Р8, EN func-набор,
  перенумерация уровней TESTING.

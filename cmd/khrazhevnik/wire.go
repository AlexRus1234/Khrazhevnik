...
type App struct {
	...
	// без подписи (trusted=yes), роут /key.asc не регистрируется
	// (wildcard → 404).
	Signer port.Signer
...
// подписи, роут /key.asc не регистрируется — 404). Но ошибка
// ИНИЦИАЛИЗАЦИИ фатальна

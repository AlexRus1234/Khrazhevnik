...
Краткая модель угроз (постаудит, сессии 19–26). Разбор конкретных
...
| админский | :30202 | loopback | REST `/api/v1`, метрики `/metrics`, SPA `/ui` |
...
Админский по умолчанию слушает loopback; публикация наружу — осознанное
...
- Квоты личных репо (bytes/files) и лимит одного объекта; перезапись
  существующего ключа — 409 (`force` — только админ, с аудитом).
